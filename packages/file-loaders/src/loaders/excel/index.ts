...
/**
 * Loads Excel files (.xlsx, .xls) from a buffer using the 'xlsx' library.
 * Each sheet becomes a DocumentPage containing a Markdown table generated by sheetToMarkdownTable.
 */
export const loadExcelFromBuffer = (dataBuffer: Uint8Array | ArrayBuffer): DocumentPage[] => {
  log('Parsing Excel buffer');
  const buffer =
    dataBuffer instanceof Uint8Array ? Buffer.from(dataBuffer) : Buffer.from(dataBuffer);
  ...
  for (const sheetName of workbook.SheetNames) {
    ...
    pages.push({
      charCount: tableMarkdown.length,
...
export class ExcelLoader implements FileLoaderInterface {
  async loadPages(filePath: string): Promise<DocumentPage[]> {
    ...
    try {
      ...
      return loadExcelFromBuffer(dataBuffer);
    } catch (e) {
